...
function catch_that_image( $str ) { 
  ...
  return $matches[1][0]; 
     ...
     function getJoomlaCategoryName($id){
        $db = JFactory::getDbo();
    ...
    function getUserName($id){
     ...
     function getJoomlaContentRate($id){
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query 
            ->select('*')
...
function seo_friendly_urls($string){
...
/**
 * Helper for mod_joomla_news
 * 
 * @package     Joomla.Site
 * @subpackage  mod_joomla_news
 * @since       1.5
 */
class ModjoomlaNEWSHelper
{
	/*
	 * @since  1.5
	 */
	...
	{ 
		...
		$query->select('*')
			->from('#__content')
			->where('`state` = 1')
			->order(''.$params->get('ordering','created').' '.$params->get('chinesh','DESC'));
			if($catid)
			$query->where("catid IN(".($catid).")");
			if($featured)
			$query->where("`featured` = '1'");
			$baze = $params->get('baze');
			if($baze=='week')
			$query->where("WEEKOFYEAR(created)=WEEKOFYEAR(NOW())");
			if($baze=='day')
			$query->where("YEAR(created) = YEAR(NOW()) AND MONTH(created) = MONTH(NOW()) AND DAY(created) = DAY(NOW())");
			if($baze=='month')
			$query->where("YEAR(created) = YEAR(NOW()) AND MONTH(created)=MONTH(NOW())");
			$selectid = $params->get('selectid','0');
			$selectid = is_array($selectid)?implode(',',$selectid):(int)$selectid;
			if((int)$selectid>'0'){
			$query->where("id IN(".($selectid).")");
			}
		//echo $query;
		...
		$menu = $app->getMenu();
		$item = $menu->getItems('link', 'index.php?option=com_content&view=category&layout=blog&id='.$catid, true);
		...
		foreach ($rows as $row)
		{
			...
			$lists[$i]->link = JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catid, $row->language));
			$lists[$i]->id = $row->id;
			$lists[$i]->text = $row->title;
			$lists[$i]->catid = $row->catid;
			$lists[$i]->desc = $row->introtext;
			$lists[$i]->full = $row->fulltext;
			$lists[$i]->hits = $row->hits;
			$lists[$i]->images = json_decode($row->images);
			$lists[$i]->urls = json_decode($row->urls);
			$lists[$i]->photo = catch_that_image( $row->images );
			if($lists[$i]->images >'0'){
			foreach ($lists[$i]->images as $post){
				$lists[$i]->photo = $lists[$i]->images->image_intro;
				$lists[$i]->photofull = $lists[$i]->images->image_intro;
			break;	
			}
			}
			if($lists[$i]->photo==''){
				$lists[$i]->photo = catch_that_image( $str );
				$lists[$i]->photofull = catch_that_image( $str2 );
			}
			if($lists[$i]->photo == ''){
				$lists[$i]->photo='modules/mod_joomla_news/assets/images/nophoto.png';
			}
			$dir="cache/dima";
			$layout = str_replace('_:','',$params->get('layout'));
			if($lists[$i]->photo > '0' and file_exists($lists[$i]->photo)){
				$width  = $params->get('width', '145');
				$height = $params->get('height', '145');
				$lists[$i]->thumb =$dir.'/'.$layout.'_'.$lists[$i]->id.'_'.str_replace('/','_',$lists[$i]->photo);
				//list($current_width, $current_height) = getimagesize($lists[$i]->photo);
				if (file_exists($lists[$i]->thumb)) {
				}else{
					$image = new SimpleImages();
					$image->load($lists[$i]->photo);
					$image->resize($width, $height);
					$image->save($lists[$i]->thumb);
				}
			}elseif($lists[$i]->photo > '0'){
				$lists[$i]->thumb=$lists[$i]->photo;
			}
			$lists[$i]->featured = $row->featured;
			$lists[$i]->id = $row->id;
			$lists[$i]->comment = '0';
			$lists[$i]->date = $row->created;
			$lists[$i]->star = $row->hits;
			$lists[$i]->attribs = $row->attribs;
			$lists[$i]->itemid = $params->get('itemid');
			$lists[$i]->category = $params->get('catid');
			$attributes = json_decode($row->attribs);
			if(isset($attributes->custom_1)){
			$lists[$i]->subtitle = $attributes->custom_1;
			}else{
				$lists[$i]->subtitle='';
			}
			$lists[$i]->alias = $row->alias;
			$lists[$i]->catid = $row->catid;
			if(isset(getJoomlaCategoryName($row->catid)->title)){
			$lists[$i]->category = getJoomlaCategoryName($row->catid)->title;
			}else{
				$lists[$i]->category='';
			}
			if(isset(getJoomlaContentRate($row->id)->rating_sum)){
			$lists[$i]->rating = round(getJoomlaContentRate($row->id)->rating_sum/getJoomlaContentRate($row->id)->rating_count);
			}
			if(isset(getUserName($row->created_by)->name)){
			$lists[$i]->owner = getUserName($row->created_by)->name;
			}else{
				$lists[$i]->owner='';
			}
			$i++;
