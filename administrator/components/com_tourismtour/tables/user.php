 ...
 * @since		1.5
 */
class tourismtourTableuser extends JTable
{
	//==========================================
	...
	function store($updateNulls = false){
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('user', 'tourismtourTable');
			if (!$oldrow->load($this->id) && $oldrow->getError())
			{
				$this->setError($oldrow->getError());
			}
			$table = JTable::getInstance('user', 'tourismtourTable');
			// Store the new row
			parent::store($updateNulls);
		}
	...
	/**
	 *
	 * @param	mixed	An optional array of primary user values to update.  If not
	 */
	public function publish($pks = null, $published = 1, $userId = 0){
		...
		// If there are no primary users set check to see if the instance user is set.
		if (empty($pks))
		{
			if ($this->$k) {
				$pks = array($this->$k);
