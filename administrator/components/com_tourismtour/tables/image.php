 ...
 * @since		1.5
 */
class tourismtourTableimage extends JTable
{
	//==========================================
	function __construct(&$_db){
		global $option;
		parent::__construct("#__tourismtour_image",'id', $_db);
	...
	function store($updateNulls = false){
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('image', 'tourismtourTable');
			if (!$oldrow->load($this->id) && $oldrow->getError())
			{
				$this->setError($oldrow->getError());
			}
			$table = JTable::getInstance('image', 'tourismtourTable');
			// Store the new row
			parent::store($updateNulls);
		}
	...
	/**
	 *
	 * @param	mixed	An optional array of primary image values to update.  If not
	 */
	public function publish($pks = null, $published = 1, $userId = 0){
		...
		// If there are no primary images set check to see if the instance image is set.
		if (empty($pks))
		{
			if ($this->$k) {
				$pks = array($this->$k);
