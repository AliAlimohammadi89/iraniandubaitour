 ...
 * @since		1.5
 */
class tourismtourTabletour extends JTable
{
	//==========================================
	function __construct(&$_db){
		global $option;
		parent::__construct("#__tourismtour_tour",'id', $_db);
	...
	function check(){
        ...
        $this->fld_points=implode(',',$this->fld_points);
        $this->fld_tourfeature=implode(',',$this->fld_tourfeature);
        $this->fld_weekday=implode(',',$this->fld_weekday);
		//print_r($this);
	...
	function store($updateNulls = false){
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('tour', 'tourismtourTable');
			if (!$oldrow->load($this->id) && $oldrow->getError())
			{
				$this->setError($oldrow->getError());
			}
			$table = JTable::getInstance('tour', 'tourismtourTable');
			// Store the new row
			parent::store($updateNulls);
		}
	...
	/**
	 *
	 * @param	mixed	An optional array of primary tour values to update.  If not
	 */
	public function publish($pks = null, $published = 1, $userId = 0){
		...
		// If there are no primary tours set check to see if the instance tour is set.
		if (empty($pks))
		{
			if ($this->$k) {
				$pks = array($this->$k);
