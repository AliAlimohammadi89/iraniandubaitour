 ...
 * @since		1.5
 */
class tourismtourTableturningpoint extends JTable
{
	//==========================================
	function __construct(&$_db){
		global $option;
		parent::__construct("#__tourismtour_turningpoint",'id', $_db);
	...
	function store($updateNulls = false){
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('turningpoint', 'tourismtourTable');
			if (!$oldrow->load($this->id) && $oldrow->getError())
			{
				$this->setError($oldrow->getError());
			}
			$table = JTable::getInstance('turningpoint', 'tourismtourTable');
			// Store the new row
			parent::store($updateNulls);
		}
	...
	/**
	 *
	 * @param	mixed	An optional array of primary turningpoint values to update.  If not
	 */
	public function publish($pks = null, $published = 1, $userId = 0){
		...
		// If there are no primary turningpoints set check to see if the instance turningpoint is set.
		if (empty($pks))
		{
			if ($this->$k) {
				$pks = array($this->$k);
