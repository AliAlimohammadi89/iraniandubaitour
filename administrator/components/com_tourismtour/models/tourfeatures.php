...
class tourismtourModeltourfeatures extends JModelList{
	//==========================================
	...
	protected function getListQuery(){
		...
		$query->from("#__tourismtour_tourfeature AS a");
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where("(a.`fld_title` LIKE $search OR a.`fld_icon` LIKE $search OR a.`fld_metadata` LIKE $search OR a.`fld_information` LIKE $search)");
			}
		}
