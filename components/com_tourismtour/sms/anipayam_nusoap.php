...
//=======================================	
	...
	function LoadSmsPanelDorCab($mode,$sms_username,$sms_password,$sms_number,$sms_center,$sms_NO=NULL,$message=NULL){
		...
		{
			$_SESSION["AniPayamcheck"] = " <strong>تنظیمات پیامک صحیح نیست</strong> ";
			return 0;
		}
		//error_reporting(0);
		try
		{
			$soapClientObj = new webserviceAniPayam($sms_center);
		...
		case 'check':
			...
			if(empty($res)){
			$_SESSION["AniPayamcheck"] = " <strong>عدم دسترسی به سرور پیامک 2</strong> ";
			return 0;
			}
			$_SESSION["AniPayamcheck"] = 1;
			return ((int)$res);
		break;
		case 'inbox':
