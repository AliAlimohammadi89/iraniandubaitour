<?php
	$GLOBALS['_transient']['static']['nusoap_sms_base']['globalDebugLevel'] = 9;
	if(!class_exists("nusoap_sms_base")){
	class nusoap_sms_base {
		var $title = 'NuSOAP';
		var $version = '0.9.5';
		...
		var $debugLevel;
		var $XML_sms_SchemaVersion = 'http://www.w3.org/2001/XML_sms_Schema';
		var $soap_defencoding = 'UTF-8';
		var $namespaces = array(
			'SOAP-ENV' => 'http://schemas.xmlsoap.org/soap/envelope/',
			'xsd' => 'http://www.w3.org/2001/XML_sms_Schema',
			'xsi' => 'http://www.w3.org/2001/XML_sms_Schema-instance',
			'SOAP-ENC' => 'http://schemas.xmlsoap.org/soap/encoding/'
			);
		var $usedNamespaces = array();
		var $typemap = array(
		'http://www.w3.org/2001/XML_sms_Schema' => array(
		...
		'http://www.w3.org/2000/10/XML_sms_Schema' => array(
			'i4'=>'','int'=>'integer','boolean'=>'boolean','string'=>'string','double'=>'double',
			'float'=>'double','dateTime'=>'string',
			'timeInstant'=>'string','base64Binary'=>'string','base64'=>'string','ur-type'=>'array'),
		'http://www.w3.org/1999/XML_sms_Schema' => array(
			'i4'=>'','int'=>'integer','boolean'=>'boolean','string'=>'string','double'=>'double',
			'float'=>'double','dateTime'=>'string',
			'timeInstant'=>'string','base64Binary'=>'string','base64'=>'string','ur-type'=>'array'),
		...
		function nusoap_sms_base() {
			$this->debugLevel = $GLOBALS['_transient']['static']['nusoap_sms_base']['globalDebugLevel'];
		}
		function getGlobalDebugLevel() {
			return $GLOBALS['_transient']['static']['nusoap_sms_base']['globalDebugLevel'];
		}
		function setGlobalDebugLevel($level) {
			$GLOBALS['_transient']['static']['nusoap_sms_base']['globalDebugLevel'] = $level;
		}
		...
		function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soap_sms_val=false) {
			$this->debug("in serialize_val: name=$name, type=$type, name_ns=$name_ns, type_ns=$type_ns, use=$use, soap_sms_val=$soap_sms_val");
			$this->appendDebug('value=' . $this->varDump($val));
			$this->appendDebug('attributes=' . $this->varDump($attributes));
			
			if (is_object($val) && get_class($val) == 'soap_sms_val' && (! $soap_sms_val)) {
				$this->debug("serialize_val: serialize soap_sms_val");
				$xml = $val->serialize($use);
				$this->appendDebug($val->getDebug());
				$val->clearDebug();
				$this->debug("serialize_val of soap_sms_val returning $xml");
				return $xml;
			}
			if (is_numeric($name)) {
			...
			if($type != '' && isset($this->typemap[$this->XML_sms_SchemaVersion][$type])){
			...
			switch(true) {
				...
				case is_object($val):
					...
					if (get_class($val) == 'soap_sms_val') {
						$this->debug("serialize_val: serialize soap_sms_val object");
				...
				case (is_array($val) || $type):
					...
					if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){
						...
						if(is_array($val) && count($val)> 0){
							foreach($val as $v){
								if(is_object($v) && get_class($v) ==  'soap_sms_val'){
									$tt_ns = $v->type_ns;
									$tt = $v->type;
								} elseif (is_array($v)) {
							...
							if(count($array_types) > 1){
							...
							} elseif(isset($tt) && isset($this->typemap[$this->XML_sms_SchemaVersion][$tt])) {
								if ($tt == 'integer') {
									$tt = 'int';
								}
								...
								$array_typename = 'unnamed_struct_use_soap_sms_val';
							} else {
	...
	if(!class_exists("nusoap_sms_fault")){
	class nusoap_sms_fault extends nusoap_sms_base {
		var $faultcode;
		var $faultactor;
		var $faultstring;
		var $faultdetail;
		function nusoap_sms_fault($faultcode,$faultactor='',$faultstring='',$faultdetail=''){
			parent::nusoap_sms_base();
	...
	if(!class_exists("soap_sms_fault")){
	class soap_sms_fault extends nusoap_sms_fault {
	}
	}
	if(!class_exists("nusoap_xmlschema")){
	class nusoap_xmlschema extends nusoap_sms_base  {
		var $schema = '';
		var $xml = '';
			...
			if(count($attrs) > 0){
				foreach($attrs as $k => $v){
					if(preg_match('/^xmlns/',$k)){
						...
						if($v == 'http://www.w3.org/2001/XML_sms_Schema' || $v == 'http://www.w3.org/1999/XML_sms_Schema' || $v == 'http://www.w3.org/2000/10/XML_sms_Schema'){
							$this->XML_sms_SchemaVersion = $v;
			...
			switch($name){
				...
				case 'attribute':
					...
					if(isset($attrs['http://schemas.xmlsoap.org/wsdl/:arrayType']) || $this->getLocalPart($aname) == 'arrayType'){
						...
						if(!strpos($v,':') && isset($this->typemap[$this->XML_sms_SchemaVersion][$v])){
							$v = $this->XML_sms_SchemaVersion.':'.$v;
						}
		...
		function serializeSchema(){
			...
			$schemaPrefix = $this->getPrefixFromNamespace($this->XML_sms_SchemaVersion);
			$xml = '';
		...
		function getTypeDef($type){
			...
			if((! $is_element) && isset($this->complexTypes[$type])){
				...
				if (!isset($this->elements[$type]['phpType'])) {
					...
					} elseif ($ns == 'http://www.w3.org/2001/XML_sms_Schema') {
				...
				$typeDef['type'] = 'http://www.w3.org/2001/XML_sms_Schema:string';
				return $typeDef;
			}
	...
	if(!class_exists("XML_sms_Schema")){
	class XML_sms_Schema extends nusoap_xmlschema {
	}
	}
	if(!class_exists("soap_sms_val")){
	class soap_sms_val extends nusoap_sms_base {
		var $name;
		var $type;
		var $value;
		var $element_ns;
		var $type_ns;
		var $attributes;
		function soap_sms_val($name='soap_sms_val',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false) {
			parent::nusoap_sms_base();
			$this->name = $name;
			$this->type = $type;
			$this->value = $value;
	...
	if(!class_exists("soap_sms_transport_http")){
	class soap_sms_transport_http extends nusoap_sms_base {
		...
		function soap_sms_transport_http($url, $curl_options = NULL, $use_curl = false){
			parent::nusoap_sms_base();
	...
	//		}
		...
		function sendRequest($data, $cookies = NULL) {
		  ...
		  if ($this->io_method() == 'socket') {
			...
			// some servers refuse to moshavereh with (so we no longer use this method!)
	...
	//		}
			...
			if(isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != ''){
				if(strtolower($this->incoming_headers['content-encoding']) == 'deflate' || strtolower($this->incoming_headers['content-encoding']) == 'gzip'){
					// if decoding moshaverehs, use it. else assume data wasn't gzencoded
	...
	class nusoap_server extends nusoap_sms_base {
		var $headers = array();
		var $request = '';
		var $requestHeaders = '';
		...
		function serialize_return() {
			...
			// if fault
			if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'soap_sms_fault') || (get_class($this->methodreturn) == 'nusoap_sms_fault'))) {
		...
		function parseRequest($headers, $data) {
			...
			// else successfully parsed request into soap_sms_val object
			} else {
				// get/set methodname
		...
		function fault($faultcode,$faultstring,$faultactor='',$faultdetail=''){
			...
			$this->fault = new nusoap_sms_fault($faultcode,$faultactor,$faultstring,$faultdetail);
	...
	class wsdl extends nusoap_sms_base {
		var $wsdl; 
		var $schemas = array();
		var $currentSchema;
		var $message = array();
		...
		function parseWSDL($wsdl = '') {
			...
			if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
				...
				// get wsdl
				$tr = new soap_sms_transport_http($wsdl, $this->curl_options, $this->use_curl);
		...
		function webDescription(){
			...
			$b = '
			<html><title>NuSOAP: '.$this->serviceName.'</title>
			<style type="text/css">
				...
				foreach($this->messages as $msgName => $msgParts) {
					...
					if(is_array($msgParts)){
						foreach($msgParts as $partName => $partType) {
							// print 'serializing '.$partType.', sv: '.$this->XML_sms_SchemaVersion.'<br>';
							if (strpos($partType, ':')) {
								$typePrefix = $this->getPrefixFromNamespace($this->getPrefix($partType));
							} elseif (isset($this->typemap[$this->namespaces['xsd']][$partType])) {
								// print 'checking typemap: '.$this->XML_sms_SchemaVersion.'<br>';
								$typePrefix = 'xsd';
							} else {
		...
		function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
			$this->debug("in serializeRPCParameters: operation=$operation, direction=$direction, XML_sms_SchemaVersion=$this->XML_sms_SchemaVersion, bindingType=$bindingType");
		...
		function serializeParameters($operation, $direction, $parameters){
			$this->debug("in serializeParameters: operation=$operation, direction=$direction, XML_sms_SchemaVersion=$this->XML_sms_SchemaVersion"); 
			$this->appendDebug('parameters=' . $this->varDump($parameters));
		...
		function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false){
			...
			if (is_object($value) && get_class($value) == 'soap_sms_val') {
				if ($value->type_ns) {
					$type = $value->type_ns . ':' . $value->type;
					$forceType = true;
					$this->debug("in serializeType: soap_sms_val overrides type to $type");
				} elseif ($value->type) {
					$type = $value->type;
					$forceType = true;
					$this->debug("in serializeType: soap_sms_val overrides type to $type");
					...
					$forceType = false;
					$this->debug("in serializeType: soap_sms_val does not override type");
				}
				$attrs = $value->attributes;
				$value = $value->value;
				$this->debug("in serializeType: soap_sms_val overrides value to $value");
				if ($attrs) {
					if (!is_array($value)) {
						$value['!'] = $value;
					}
					...
					$this->debug("in serializeType: soap_sms_val provides attributes");
				}
			} else {
				$forceType = false;
			}
			...
			if (strpos($type, ':')) {
				...
				if($ns == $this->XML_sms_SchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
			...
			if ($phpType == 'struct') {
				...
				if (is_array($value) && sizeof($value) >= 1) {
					...
					foreach($value as $k => $v) {
						...
						if (!in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XML_sms_Schema'])) {
							$contents .= $this->serializeType('item', $typeDef['arrayType'], $v, $use);
						} else {
							$contents .= $this->serialize_val($v, 'item', $typeDef['arrayType'], null, $this->XML_sms_SchemaVersion, false, $use);
						} 
					}
				} else {
					$rows = 0;
	...
	class nusoap_parser extends nusoap_sms_base {
		...
		function start_element($parser, $name, $attrs) {
			...
			foreach($attrs as $key => $value){
				...
				if($key_prefix == 'xmlns'){
					if(preg_match('/^http:\/\/www.w3.org\/[0-9]{4}\/XML_sms_Schema$/',$value)){
						$this->XML_sms_SchemaVersion = $value;
						$this->namespaces['xsd'] = $this->XML_sms_SchemaVersion;
						$this->namespaces['xsi'] = $this->XML_sms_SchemaVersion.'-instance';
					}
	...
	class nusoap_client extends nusoap_sms_base  {
			...
			switch(true){
				case preg_match('/^http/',$this->endpoint):
					...
					if($this->persistentConnection == true && is_object($this->persistentConnection)){
						...
						$http = new soap_sms_transport_http($this->endpoint, $this->curl_options, $this->use_curl);
						if ($this->persistentConnection) {
