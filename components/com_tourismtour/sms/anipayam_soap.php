<?php
//=======================================	CONCAT(dv.vLicencePlate,'@',dv.vLicenceSeriPlate) AS vLicencePlate
	class webserviceAniPayam_Soap{
	...
	public function GetCredit(){
		...
		$result = 0;
		try {
			$result = $this->client_object->GetCredit($args)->GetCreditResult;
			return $result;
		} catch (SoapFault $fault) {
			return "$fault";
		}
...
//=======================================	
	...
	function LoadSmsPanelDorCab($mode,$sms_username,$sms_password,$sms_number,$sms_center,$sms_NO=NULL,$message=NULL){
		...
		try
		{
			$soapClientObj = new webserviceAniPayam_Soap($sms_center);
